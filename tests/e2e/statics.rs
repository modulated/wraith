...
fn static_scalar_read_write() {
    ...
    "#);
    assert_eq!(e.mem(0x0400), 8, "static initialized to 7 then incremented");
}
...
fn static_initial_value_written_at_reset() {
    ...
    "#);
    assert_eq!(e.mem(0x0400), 0xA5, "declared initial value present at startup");
}
...
fn static_persists_across_function_calls() {
    ...
    "#);
    assert_eq!(e.mem(0x0400), 15, "static accumulated across three calls");
}
...
fn two_statics_do_not_overlap() {
    let mut e = run(r#"
        static A: u8 = 1;
        static B: u8 = 2;
        const OUT_A: addr = 0x0400;
        const OUT_B: addr = 0x0401;
        #[reset]
        fn main() {
            A = 0x11;
            B = 0x22;
    ...
    "#);
    assert_eq!(e.mem(0x0400), 0x11, "A unaffected by writing B");
    assert_eq!(e.mem(0x0401), 0x22, "B holds its own value");
}
...
fn static_u16_holds_both_bytes() {
    ...
    "#);
    assert_eq!(e.mem16(0x0400), 0x1234, "u16 static keeps both bytes");
}
...
fn irq_handler_and_main_share_a_static() {
    ...
    // reads back directly (BSS starts at $6000).
    let mut e = run(r#"
        static IRQ_COUNT: u8 = 0;
        const READY: addr = 0x0400;
        #[irq]
        fn on_irq() {
    ...
    "#);
    assert_eq!(e.mem(0x0400), 1, "reached the idle loop");
    assert_eq!(e.mem(0x6000), 0, "shared counter starts at its initializer");

    e.pulse_irq();
    assert_eq!(e.mem(0x6000), 1, "handler incremented the shared static");

    e.pulse_irq();
    e.pulse_irq();
    assert_eq!(e.mem(0x6000), 3, "each interrupt updates shared state");
}
...
fn static_array_indexed_read_write() {
    ...
    "#);
    assert_eq!(e.mem(0x0400), 0x11, "BUF[0] written and read back");
    assert_eq!(e.mem(0x0401), 0x5A, "BUF[i] with a runtime index");
}
