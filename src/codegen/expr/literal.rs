...
pub(super) fn generate_literal(
    ...
    match lit {
        ...
        // A generated table is data, never a value in a register: the sites
        // that lay one out (`init.rs` for ROM and startup images, the local
    ...
    if let Some(sym) = info.resolved_symbols.get(&span) {
        ...
        match sym.location {
            SymbolLocation::FrameOffset(_) => Err(CodegenError::Internal(
                "unresolved FrameOffset reached codegen (frame finalization skipped)".to_string(),
            )),
