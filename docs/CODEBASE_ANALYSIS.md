...
_Update: all 22 criticals and every HIGH item are fixed, each with regression tests. Most MEDIUM items are fixed as of 2026-07-30 as well (see the struck-through entries); what remains open there is the exhaustive-walker refactor, CG-M6–M8 (stdlib asm size), sema-level multi-error reporting, string-matching test pockets, the interrupt stack-depth check, and the LOW polish list._
...
- **~~CG-M1. Comparisons materialize a boolean, then conditions re-test it~~ FIXED** — a flag-liveness-guarded peephole (`collapse_boolean_compares`) folds the materialize-then-retest shape into a single conditional branch; `if (x < 3)` went from 13 instructions to 4.
- **~~CG-M2. Literal right operands round-trip through `$20`~~ FIXED** — `fold_literal_operand` emits `OP #$imm`, applied only when a forward scan proves the staged `$20` dead (a match's scrutinee read vetoes it). The fold exposed that `x + 0`'s "NOP elimination" test only passed because the operand was hidden in `$20`; `CLC; ADC #$00` / `SEC; SBC #$00` pairs are now genuinely removed when no flag is live.
- **~~CG-M3. `JMP` to the next instruction~~ FIXED** — trivial peephole.
- **~~CG-M4. Argument staging double-store for simple args~~ FIXED** — all-trivial 8-bit args (literal/variable) to a non-recursive, non-address-taken callee store straight into the callee's frame (`add(3, x)`: 9 instructions → 5).
- **~~CG-M5. Interrupt prologue saves 60 zero-page bytes unconditionally~~ FIXED** — `finalize_frames` scans each handler's reachable call graph: scratch is saved only when the handler stages anything, `$D0–$DC` only when it does 16-bit mul/div/mod. A trivial handler saves nothing. (The hardware-stack depth check for interrupts remains open.)
- **CG-M6/M7/M8.** stdlib math ROM over-allocation (~49 bytes across mul16/div16/mod16); copy loops unrolled per byte (6 code bytes per byte copied — a DEX/BNE loop pays off beyond ~3 bytes); `LDA #$00; LDY #$00` → `LDA #$00; TAY`. *Open.*

### Structure & maintainability

- **The dominant defect pattern is per-form walkers and merge lists.** *Partly fixed:* the import merge is now a single `merge_imported` method with an explicit not-merged list, pointed at from the `SemanticAnalyzer` struct, and `with_base_path` delegates to `new()` (the two 40-field initializer copies are gone). The exhaustive Stmt/Expr recursion helper shared by all analysis walkers remains open.
- **~~Test-suite dedup~~ FIXED** — `feature_tests.rs` and `codegen_tests.rs` (weaker, string-matching duplicates of the e2e suites) deleted; `error_tests.rs` kept as the only diagnostics-message suite; dead `tests/common/fixtures.rs` and 20 unreferenced `.wr` fixtures + `.asm`/`.o` debris removed.
- **String-matching pockets inside e2e** (`cpu_flags.rs`, much of `frames.rs`/`types.rs`/`control_flow.rs`/`memory.rs`) should become behavioral assertions where behavior is assertable. *Open.*
- **~~Emulator fidelity~~ FIXED** — `TestBus` panics on a store into the CODE/DATA (ROM) ranges instead of silently accepting it (with a proving test through a pointer), and `run_with_devices` asserts halt like `run` (a program meant to spin on a device uses `run_with_devices_expect_spin`).
- **~~std coverage gaps~~ FIXED** — execution tests for `set_bit`/`clear_bit`/`test_bit`, `saturating_*`, `count_bits`, `reverse_bits`, `swap_nibbles`, `mem_read`/`mem_write`/`mem_jump`, and the intrinsics. The coverage found three live bugs: `swap_nibbles` dropped its last ROL, `reverse_bits`/`swap_nibbles` staged through the compiler's `$20` pool (both are plain Wraith now), and `fn as u16` zero-extended a code address's real high byte.
- **~~FE-M1–M6~~ FIXED** — `#[org(0x10000)]` is a parse error instead of truncating; out-of-range integer literals lex as "integer literal … is too large"; flatasm errors on duplicate labels/symbols, emission past `$FFFF`, and `.BYTE 300` (the byte at `$FFFF` stays legal for the vector table); lexer errors render with source carets; `wraith.toml` with `end < start` or a missing `default_section` is rejected at load. (Sema still reports only the first error — parser-level multi-error exists; sema-level multi-error remains open.)
- **~~SE-M3/M4~~ FIXED** — the unused and under-counting `calculate_size` helpers are deleted (`sema::init::size_of` is the single source), and the never-pushed `SemanticAnalyzer.errors` field is gone. The interrupt-save docs now match reality: the save set is computed (CG-M5).
- **~~DOCS-M10–M20~~ FIXED** — spec: struct passing documented as by-reference, string `==` documented, "no implicit conversions" replaced with the actual rules, keyword list corrected (38, `static` added), output described as flat absolute image; `#[interrupt]` is now a hard error pointing at `#[irq]`/`#[nmi]`; `WRAITH_STD_PATH` documented; `syntax_extension` keyword lists corrected; `examples/monitor_standalone.wr` rebuilt (missing UART layer restored, command buffer moved off the software-stack page, scratch-pool-colliding addr consts removed) and covered by an end-to-end test against the 16550 model; `std/math.wr` bit helpers are NMOS-legal pure Wraith (see CG entry above). `docs/TESTING.md`, `docs/FEATURES_REVIEW.md` and `axon.cfg` were already gone.
