...
- **Hardware Numeric Types** - beyond `u8`/`i8`/`u16`/`i16`: `b8`/`b16` binary-coded decimal use the 6502's decimal mode for exact base-10 arithmetic (scores, clocks, 7-segment output), and `q8.8` is signed 8.8 fixed-point for fractional values — positions, velocities, angles — where `let v: q8.8 = 1.5;` is scaled at compile time and add/subtract are plain 16-bit ops, no software float
