...
| `Field` | 291 | two scalar fields of the program's type, one array field, and a nested struct `n: Inner` with two scalars of its own — reached through the struct's own name in `main` and through the by-reference parameter inside the function that takes it, `xp.a[i]` included, which indexes an array field through a pointer. The nested field is read as `s.n.g{j}` in `main` — a two-level offset — but not through the parameter |
| `Index` | 313 | the index is a constant, a loop variable, or `(v as u8) % 4` — always in range, because the language does no bounds checking and an out-of-range access would be the generator's bug rather than the compiler's |
...
| `Match` | 107 | as a statement, the whole-body `ViaMatch` form; as an expression, the enum payload read `match e { C::C0 => …, C::C{k}(x) => (x as T) }`, whose arms all share the context type so no arm-type unification is exercised. A match over integers or ranges as a value is not generated |
...
| `Sub` | 352 |  |
| `Mul` | 271 |  |
| `Div` | 295 | divisor is any expression, zero included — `x / 0` is the all-ones sentinel the specification defines, and `i8::MIN / -1` wraps like every other overflow. Only a *constant* divisor is excluded, since a zero one is a compile error |
| `Mod` | 347 | divisor as for `Div` |
| `BitAnd` | 315 |  |
| `BitOr` | 315 |  |
| `BitXor` | 293 |  |
| `Shl` | 310 | shift count is a literal below the type's width, where the result is the plain shift |
| `Shr` | 294 | shift count as for `Shl` |
| `Eq` | 327 |  |
| `Ne` | 101 |  |
| `Lt` | 83 |  |
| `Gt` | 167 |  |
